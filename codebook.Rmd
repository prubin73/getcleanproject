...
Assuming that the grouped means are in a `tbl_df` object, and that the `dplyr` package is loaded, means of a specific original variable can be accessed by filtering on the components of the original variable name. (Be sure to specify all seven components, including ones missing from the given variable name.)

### Examples
+ To access means for *tBodyAcc-mean()-X*, use the following code snippet:
```{r, eval = FALSE}
groupMeans                           %>%
  filter(Domain == "time",                  # 't'
         Component == "Body",               # 'Body'
         Device == "accelerometer",         # 'Acc'
         Statistic == "mean",               # 'mean()'
         Direction == "X",                  # 'X'
         Jerk == FALSE,                     # 'Jerk' omitted
         Magnitude == FALSE)         %>%    # 'Mag' omitted
  select(Subject, Activity, Mean)
```
+ To access means for *fBodyBodyGyroJerkMag-std()*, use the following:
```{r, eval = FALSE}
groupMeans                                   %>%
  filter(Domain == "frequency",                    # 'f'
         Component == "BodyBody",                  # 'BodyBody'
         Device == "gyroscope",                    # 'Gyro'
         Statistic == "standard_deviation",        # 'std()'
         Direction == NA,                          # no X, Y or Z
         Jerk == TRUE,                             # 'Jerk'
         Magnitude == TRUE)                  %>%   # 'Mag'
  select(Subject, Activity, Mean)
```
